...
class TranseporterProfile extends StatefulWidget {
//user.Usr logeduser;
  String farmer_id;
  int fid;
  Usr userfarmer;
  TranseporterProfile({this.farmer_id, this.fid, this.userfarmer});

  @override
  _ProfileState createState() => _ProfileState();
}

class _ProfileState extends State<TranseporterProfile> {
...
//final String url='https://www.cropscience.bayer.com/-/media/bcs-inter/ws_globalportal/crop-science/smallholder-farming/smallholder-farming-in-india.jpg';
  ...
  Future _profileinfo() async {
    String url = 'http://34.87.38.40/transporter/' + widget.farmer_id.toString();
    ...
    await http.get(url).then((response) {
      ...
      if (response.statusCode == 200) {
        ...
        if (content == '[]') {
          NoItempopup(this.context,'No Info about ');
        } else {
          setState(() {
            //approved = json['confirmed'];
            //buyerlist = prdct.map((json) => Usr.fromJson(json)).toList();
            userfarmer = Usr.fromJsontransporter(json);
            isloading = false;
          });
                                  ...
                                  child: ClipOval(
                                    child: SizedBox(
                                      ...
                                      child: (_image != null)
                                          ...
                                          //     fit: BoxFit.fill,
                                          //   ),
                                          :CircleAvatar(child: Text(userfarmer.fullname[0]),)
                                    ),
                                  ),
                                )),
                          ),
