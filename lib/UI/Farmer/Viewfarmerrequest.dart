...
class _ViewrequestState extends State<Viewrequest> {
  ...
  reqestedbuyer() async {
    ...
    await http.get(url).then((response) {
      ...
      if (response.statusCode == 200) {
        ...
        if (content == '[]') {
          NoItempopup(this.context,'No Buyer has Requested yet');
        } else {
          setState(() {
  ...
  confirmbuyer(String buyrid) async {
    String url = 'http://34.87.38.40/purchaseItem/' +
        ...
        '/select_buyer/' +
        buyrid+'/'+myitem.farmer.id.toString();
      print(url);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(widget.myitem.type.toString()),
      ),
      body: Column(
                          ...
                          itemBuilder: (context, index) {
                            return Card(
                              ...
                              child: InkWell(
                                onTap: () {
                                  print('object');
                                  Navigator.push(context, MaterialPageRoute(builder: (context)=>Profileviewandrate(id: int.parse(widget.farmer_id),role: 'farmer',canrate: false,)));
                                },
                                child: Column(
                                  children: <Widget>[
                                    Row(
